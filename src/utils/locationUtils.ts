...
import type { Coordinates } from "./filterResources";
...
/** Approximate center coordinates for Chicagoland ZIP codes */
const ZIP_CENTERS: Record<number, Coordinates> = {
  60076: { lat: 42.032, lng: -87.747 },
  60153: { lat: 41.879, lng: -87.844 },
  60160: { lat: 41.9,   lng: -87.833 },
  60201: { lat: 42.045, lng: -87.687 },
...
function getZipCenter(query: string): Coordinates | null {
  const zip = parseInt(query.replace(/\D/g, ""), 10);
  if (query.replace(/\D/g, "").length !== 5) return null;
